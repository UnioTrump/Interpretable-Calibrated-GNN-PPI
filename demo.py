...
import time
import numpy as np
from utils import ProteinData, PPIDataLoader, WeightedCrossEntropy, calculate_metrics
...
raw_dict = load_data('/gz-data/train355-r5.5-a2.3.pkl')
...
max_residue_nodes = max([sample['residue_graph'].x.size(0) for sample in full_dataset.samples]) # 869
...
def train(run, model, loader, optimizer, grad_norm=None, delta=0.5):
    ...
    for batch in loader:
        ...
        # 前向传播
        out = model(atom_x, atom_adj_t, residue_x, residue_adj_t, a2r_map)
...
def test(model, loader):
    ...
    return total_loss / total_examples, metrics, threshold
...
for epoch in epoch_pbar:
    ...
    train_loss = train(0, model, train_loader, optimizer, grad_norm=1.0)
    train_time = time.time() - start_time

    # 在验证集上评估
    val_loss, metrics, best_threshold = test(model, val_loader)
