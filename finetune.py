...
def test(model, val_proteins, cfg):
    ...
    if not all_targets:
        ...
        return 0.0, {'pr_auc': 0.0, 'roc_auc': 0.0, 'f1': 0.0}, 0.5
...
def plot_loss_curves(train_losses, val_losses, save_path):
    ...
    plt.title('Fine-tuning: Training and Validation Loss Curves')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    ...
    print(f"Fine-tuning loss curve saved to: {save_path}")
    plt.close()

def main():
    cfg = config.FINETUNE
    print("--- Starting Fine-tuning ---")
    print(f"Loading fine-tuning data from: {cfg['DATA_PATH']}")
    ...
    print(f"Loading pre-trained model from: {cfg['PRETRAINED_PATH']}")
    ...
    epoch_pbar = tqdm(range(cfg['EPOCHS']), desc="Fine-tuning Progress", ncols=180)

    for epoch in epoch_pbar:
        train_loss = train(model, train_data, optimizer, cfg)
        train_losses.append(train_loss)

        val_loss, metrics, best_threshold = test(model, val_data, cfg)
        ...
        if pr_auc > best_pr_auc:
            ...
            print(f"\nEpoch {epoch+1}: PR AUC improved to {pr_auc:.4f}. Model saved to {cfg['MODEL_SAVE_PATH']}")
        else:
            patience_counter += 1
    ...
    plot_save_path = os.path.join(config.PLOT_DIR, f'{config.PROJECT_NAME}_finetune_loss_curve.png')
